...
**2. 父子组件通信问题**
场景： 子组件A通过props获取父组件传过来的data数据渲染页面。同时，另一个子组件B通过事件告知父组件更新data数据，子组件A更新视图。出现的问题： 子组件A并未更新。更加诡异的是，首次触发事件没问题，之后的触发才会出现。

`排查：`子组件B提交新数据回到父组件，在事件函数中打印data，发现居然还是旧的。
